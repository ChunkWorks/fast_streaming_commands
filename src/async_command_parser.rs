...
pub struct AsyncCommandParser<T, Fut>
where
    T: FnMut(&[u8]) -> Fut,
    Fut: Future<Output = ()>,
{
    ...
    pub read_until_length: u16,
}

impl<T, Fut> AsyncCommandParser<T, Fut>
where
    T: FnMut(&[u8]) -> Fut,
    Fut: Future<Output = ()>,
{
    pub fn new(callback: T) -> AsyncCommandParser<T, Fut> {
        AsyncCommandParser {
            callback,
            buffer: Vec::<u8>::new(),
            length_buffer: [0, 0],
    ...
    pub fn parse(&mut self, buf: &[u8]) {
        for byte in buf {
            if self.read_first_length_idx < 2 {
                self.length_buffer[self.read_first_length_idx as usize] = *byte;
                self.read_first_length_idx += 1;
                continue;
            }
            if self.read_until_length == 0 {
                self.read_until_length = u16::from_le_bytes(self.length_buffer);
            }
            let mut buf_len: u16 = self.buffer.len().try_into().unwrap();
